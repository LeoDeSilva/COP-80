...
//    this.MemoryChip.CreateFile("SPRITE.COP", "FN NEW_SPRITE(X, Y, W, H, C) GO\n\tRETURN {\n\t\tX: X,\t\n\t\tY: Y,\n\t\tW: W,\n\t\tH: H,\n\t\tC: C,\n\t}\nEND\n\nFN IS_SOLID(X, Y, FLAG) GO\n\tIF FGET(MGET(X,Y), FLAG) THEN\n\t\tRETURN TRUE \n\tELIF FGET(MGET(X+7, Y), FLAG) THEN\n\t\tRETURN TRUE\n\tELIF FGET(MGET(X, Y+7), FLAG) THEN\n\t\tRETURN TRUE\n\tELIF FGET(MGET(X+7 ,Y+7), FLAG) THEN\n\t\tRETURN TRUE\n\tEND\n\n\tRETURN FALSE\nEND\n\nFN DRAW_SPRITE(S) GO\n\tRECT(S.X, S.Y, S.W, S.H, S.C)\nEND\n\nFN INTERSECT(S1, S2) GO\n\tL1 = [S1.X, S1.Y]\n\tR1 = [S1.X+S1.W, S1.Y+S1.H]\n\n\tL2 = [S2.X, S2.Y]\n\tR2 = [S2.X+S2.W, S2.Y+S2.H]\n\t\n\tIF L1[0] == R1[0] OR L1[1] == R1[1] OR L2[0] == R2[0] OR L2[1] == R2[1] THEN\n\t\tRETURN FALSE\n\tEND\n\n\tIF L1[0] > R2[0] OR L2[0] > R1[0] THEN\n\t\tRETURN FALSE\n\tEND\n\n\tIF R1[1] < L2[1] OR R2[1] < L1[1] THEN\n\t\tRETURN FALSE\n\tEND\n\tRETURN TRUE\nEND")

    this.MemoryChip.CreateFile("SPRITE.COP", "FN NEW_SPRITE(X, Y, W, H, C) GO\n\tRETURN {\n\t\tX: X,\t\n\t\tY: Y,\n\t\tW: W,\n\t\tH: H,\n\t\tC: C,\n\t}\nEND\n\nFN HIT(X, Y, W, H, FLAG) GO\n\tIF FGET(MGET(X,Y), FLAG) THEN RETURN TRUE END\n\tIF FGET(MGET(X+W,Y), FLAG) THEN RETURN TRUE END\n\tIF FGET(MGET(X, Y+H), FLAG) THEN RETURN TRUE END\n\tIF FGET(MGET(X+W, Y+H), FLAG) THEN RETURN TRUE END\n\tRETURN FALSE\nEND\n\nFN IS_SOLID(X, Y, FLAG) GO\n\tIF FGET(MGET(X,Y), FLAG) THEN\n\t\tRETURN TRUE \n\tELIF FGET(MGET(X+7, Y), FLAG) THEN\n\t\tRETURN TRUE\n\tELIF FGET(MGET(X, Y+7), FLAG) THEN\n\t\tRETURN TRUE\n\tELIF FGET(MGET(X+7 ,Y+7), FLAG) THEN\n\t\tRETURN TRUE\n\tEND\n\n\tRETURN FALSE\nEND\n\nFN DRAW_SPRITE(S) GO\n\tRECT(S.X, S.Y, S.W, S.H, S.C)\nEND\n\nFN INTERSECT(S1, S2) GO\n\tL1 = [S1.X, S1.Y]\n\tR1 = [S1.X+S1.W, S1.Y+S1.H]\n\n\tL2 = [S2.X, S2.Y]\n\tR2 = [S2.X+S2.W, S2.Y+S2.H]\n\t\n\tIF L1[0] == R1[0] OR L1[1] == R1[1] OR L2[0] == R2[0] OR L2[1] == R2[1] THEN\n\t\tRETURN FALSE\n\tEND\n\n\tIF L1[0] > R2[0] OR L2[0] > R1[0] THEN\n\t\tRETURN FALSE\n\tEND\n\n\tIF R1[1] < L2[1] OR R2[1] < L1[1] THEN\n\t\tRETURN FALSE\n\tEND\n\tRETURN TRUE\nEND")
