...
function ListDirectory(Terminal) {
  for (
    let i = 0;
    i < Terminal.Kernel.MemoryChip.CurrentDirectory.SubDirs.length;
    i++
  ) {
    if (
      Terminal.Kernel.MemoryChip.CurrentDirectory.SubDirs[i].Name == ".MODULES"
    )
      continue;
...
function Load(Terminal) {
  ...
  switch (fileName) {
    case "PIANO":
    case "PIANO.COP":
      Terminal.Kernel.MemoryChip.CreateFile(
        "PIANO.COP",
    ...
    default:
      Terminal.appendHistory(
        "string",
        "ERROR: FILE NOT FOUND, EXPECTED: PIANO or SNAKE",
        14
      );
  }
}
  ...
  return [
    {
      ...
      content: (Kernel, y) => {
        ...
        return Kernel.Icons["cop-80"].Blit(Kernel.DisplayChip, 0, y);
      },
      colour: 0,
    },
    {
