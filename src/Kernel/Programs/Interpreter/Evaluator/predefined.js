...
function joinArguments(LineNumber, args) {
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i].Type != TOKENS.STRING) return [
      null,
      new Error("LINE " + LineNumber + " ARGUMENTS TO PRINT() MUST BE OF TYPE STRING, GOT: " + args[i].Type)
    ]
    str += args[i].Value
  }
...
function text(LineNumber, args, Environment) {
  ...
  if (args[0].Type != TOKENS.STRING) return checkArgument(LineNumber, "TEXT", 0, TOKENS.STRING, args)
  ...
  Environment.Kernel.FontChip.BlitText(Environment.Kernel.DisplayChip, args[0].Value, args[1].Value, args[2].Value, args[3].Value)
